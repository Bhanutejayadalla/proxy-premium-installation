...
## MESH TRANSPORT PLAN (Wi-Fi Direct Preferred + BLE Fallback)

### Goal
Use dual transport for mesh text messaging:
- If Wi-Fi Direct is ON and connected, use Wi-Fi Direct.
- If Wi-Fi Direct is unavailable/OFF, small text messages should go through BLE.

### Requested Policy
1. Wi-Fi Direct is the primary transport.
2. BLE fallback is allowed for small text messages up to 180 characters.
3. Relay forwarding is allowed under this policy.
4. Keep hop/TTL/duplicate safety controls strict.

### Implementation Plan
1. Add a transport selector in mobile_app/lib/services/mesh_service.dart:
  - Prefer Wi-Fi Direct when connected and socket path is healthy.
  - If not healthy and text length <= 180 chars, attempt BLE fallback.
2. Extend mobile_app/lib/ble_service.dart from discovery-only to payload messaging:
  - Add BLE write/notify path.
  - Add framing/chunking/reassembly for encrypted payload.
3. Integrate transport policy into direct send and relay paths in mobile_app/lib/services/mesh_service.dart.
4. Make dedup transport-aware in mobile_app/lib/services/mesh_service.dart and packet metadata in mobile_app/lib/services/mesh_encryption_service.dart to avoid duplicates in mixed BLE/Wi-Fi routes.
5. Update mesh UI in mobile_app/lib/screens/mesh_chat_screen.dart:
  - Show active transport state (Wi-Fi Direct or BLE fallback).
  - Add per-message transport badge.
6. Update README.md to clearly state runtime behavior:
  - Wi-Fi Direct preferred when available.
  - BLE fallback for <=180-char text when Wi-Fi Direct is unavailable.
  - Handshake/connection latency can still happen even when devices are nearby.

### Verification Checklist
1. Two-device test: Wi-Fi Direct OFF, <=180-char text is delivered through BLE.
2. Two-device test: Wi-Fi Direct ON, same text uses Wi-Fi Direct.
3. Three-device test: relay forwarding works under policy without loop storms.
4. Mixed-route test: no duplicate render when same message appears via BLE and Wi-Fi.
5. UI test: banner/badges show actual transport path.

### Prompt to Implement This
Implement dual-transport mesh messaging for this Flutter app.

Required behavior:
- If Wi-Fi Direct is available, always use Wi-Fi Direct for message transport.
- If Wi-Fi Direct is unavailable, allow BLE fallback for text messages up to 180 characters.
- Relay forwarding must also work under this policy, with strict TTL/hop/loop controls.

Update these files:
- mobile_app/lib/services/mesh_service.dart
- mobile_app/lib/ble_service.dart
- mobile_app/lib/services/mesh_encryption_service.dart
- mobile_app/lib/screens/mesh_chat_screen.dart
- README.md

Add:
- transport-aware dedup
- retry queueing
- UI transport indicators

Add or update tests for:
- Wi-Fi preferred routing
- BLE fallback routing for <=180 chars
- relay behavior and loop prevention
- mixed-transport dedup correctness
